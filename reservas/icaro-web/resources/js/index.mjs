...
class App {
    ...
    static async main() {
        ...
        window.createMenu = async (menu, callBack) => {
            ...
            // referencia la lista de opciones del menú cargado
    ...
    static async #checkUser() {
        try {
            ...
            let response = await Helpers.fetchData(`${urlAPI}/usuarios/autenticar`, {
                method: 'POST',
                body: {
                    perfil: document.querySelector('#nick').value,
    ...
    static async #loadUserPage() {
        ...
        switch (App.#user.tipo) {
            ...
            case 'CLIENTE':
                ...
                current = new Client(App.#showMainMenu)
                break
            case 'AUXILIAR':
                const { default: Auxiliar } = await import('./auxiliar.js')
                current = new Auxiliar(App.#showMainMenu)
                break
            default:
                break
        }
    ...
    static async #mainMenu(e) {
        ...
        switch (option) {
            ...
            case 'Tema':
                ...
                break
            default:
                console.log(`¿Qué hace usted aquí? ¿${option}? Jmmmmm...`)
        }
    }
}
